...
angular.module("mightyDatepicker").directive "mightyDatepicker", ($compile) ->
  ...
  link: ($scope, $element, $attrs) ->
    ...
    _buildMonth = (time) ->
      ...
      weeks = (_buildWeek(moment(start).add('weeks', w), moment(start).month()) for w in [0 .. weeksInMonth])
      weeks: weeks
    ...
    _prepare = ->
      $scope.months = []
      $scope.months = (_buildMonth(moment($scope.options.start).add('months', m)) for m in [0 ... $scope.options.months])
